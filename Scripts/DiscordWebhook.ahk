WindowCaptureToWebhook(image := imgSavePath) {
    ...
    {
        name:"Macro Status",
        value: displayLoops.Text "`n" runsPerHour.Text "`n" timeElapsed.Text,
        inline: true
    },
    {
        name: "Challenge Info",
        value: challengesCompleted.Text,
        inline: true
    },
    ])
    embed.setFooter({text:"yuh"})
    embed.setThumbnail({url:thumbnail.attachmentName})
    embed.setImage({url:image.attachmentName || imgSavePath})

    webhook.send({
        embeds: [embed],
        files: [image]
    })
    InsertText(processText, "Send Capture To Webhook")
}
